# Sites Públicos

Esta seção documenta todos os sites públicos da Administradora Mutual.

## Documentação Disponível

Para cada site, você encontrará:

- **Documentação Geral:** Informações sobre o site, suas seções, funcionalidades e status
- **Stack Técnica:** Detalhes sobre tecnologias, arquitetura, estrutura de código e padrões de desenvolvimento

## Sites Documentados

### 🌐 Site Institucional
- [Documentação Geral](./institucional)
- [Stack Técnica](./institucional-tech)

### 🏢 AURA Seguradora (S4)
- [Documentação Geral](./aura-s4)
- [Stack Técnica](./aura-s4-tech)

### 📋 Gestão Segura
- [Documentação Geral](./gestao-segura)

### 🎨 Manual da Marca
- [Documentação Geral](./manual-marca)

### 🏛️ Grupo MMB
- [Documentação Geral](./grupo-mmb)
- [Stack Técnica](./grupo-mmb-tech)
